...
vec3 DeduceNorm(vec2 uv)
{
    vec4 norm01 = texture(uNormalMap0, uv);
    vec3 normTan = norm01.xyz * 2.f - 1.f;
    return normalize(fsIn.TBN * normTan);
}
...
void main()
{
    vec3 tangengFragToCam = normalize(fsIn.TangentViewPos - fsIn.TangentFragPos);
    vec2 parallaxUv = ParallaxUv(fsIn.Uv0, tangengFragToCam);
    if(parallaxUv.x > 1.0 || parallaxUv.y > 1.0 || parallaxUv.x < 0.0 || parallaxUv.y < 0.0)
        discard;

    vec4 srgbColor = texture(uTexture0, parallaxUv);
    vec4 linearColor = ToLinearColor(srgbColor);

    // Calculate ambient component.
    vec3 ambientColor = uAmbientColor.w * uAmbientColor.rgb;

    vec3 norm = DeduceNorm(parallaxUv);
